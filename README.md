...
`tests/` 디렉토리에 모든 커스터마이징 파일의 동작을 검증하는 **20개 테스트 프롬프트**가 준비되어 있습니다.

| 카테고리 | 테스트 수 | 설명 |
|---------|----------|------|
| [Instructions](tests/instructions/) | 5개 | coding, communication, persona, safety, thinking 지침 검증 |
| [Prompts](tests/prompts/) | 5개 | architecture, code-review, debug, explain, refactor 프롬프트 검증 |
| [Skills](tests/skills/) | 6개 | fact-check, web-search, competitive-analysis, azure 스킬 검증 |
| [Agents](tests/agents/) | 2개 | instruction-reviewer, skill-scaffolder 에이전트 검증 |
| [Global Rules](tests/global-rules/) | 2개 | 팩트체크, 출처 표시 글로벌 규칙 검증 |

각 테스트 파일에는 **프롬프트**와 **검증 포인트 체크리스트**가 포함되어 있어, 지침이 올바르게 적용되는지 체계적으로 확인할 수 있습니다. 자세한 내용은 [tests/README.md](tests/README.md)를 참조하세요.
