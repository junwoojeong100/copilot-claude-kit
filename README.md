...
1. **Copilot Chat** 패널을 열고 입력창의 📎 (Attach) 버튼을 클릭합니다.
2. **Prompt...** 항목을 선택하면 사용 가능한 프롬프트 목록이 표시됩니다.
3. 원하는 프롬프트를 선택하면 **컨텍스트로 첨부**됩니다 (입력창 위에 태그로 표시).
4. 입력창에 **실제 질문이나 요청을 직접 입력**하고 전송합니다.

> **참고**: 프롬프트 파일의 내용은 채팅 입력창에 텍스트로 나타나지 않습니다. Copilot이 배후에서 프롬프트 지침과 사용자 입력을 합쳐서 처리합니다. 예를 들어 `code-review` 프롬프트를 첨부한 뒤 "이 파일 봐줘"라고만 입력하면, 리뷰 지침이 자동으로 적용됩니다.
