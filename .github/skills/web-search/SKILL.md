...
- 필수 조사 축과 핵심 주장에 충분한 근거가 모이면 중단하며, 결론을 바꾸지 않는 추가 검색으로 범위를 넓히지 않는다.
...
1. **범위 확정**: 결론을 바꾸는 blocking 입력만 확인한다. 그 외에는 합리적인 가정을 표시하고 진행한다.
...
- 필수 조사 축이 충족되고 미해결 충돌이 결론에 미치는 영향을 기록했으면 검색을 종료했다.
