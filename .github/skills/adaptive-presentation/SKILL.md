...
- 정보 관계에 맞는 시각 형태
- 이전·다음 슬라이드와의 논리적 연결
- 고객을 환기시키는 질문·발표자가 그대로 말할 핵심 메시지·다음 장 전환
...
- 한국어 덱은 모든 슬라이드에 speaker notes를 작성한다. 기존 notes가 있더라도 먼저 완전히 지우고
  storyline·현재 slide visual·Fact Ledger만 기준으로 새로 생성한다. 기본 형식은 아래 3개 블록이며
  `speakerNotesPolicy`가 이를 검증한다.
  - `질문:` 고객이 실제 사례와 판단 기준을 함께 떠올리게 하는 짧은 질문 한 문장
  - `핵심 메시지:` 슬라이드의 결론·작동 원리·고객 의미를 충분히 설명하는 1~3문장
  - `전환:` 현재 결론에서 남은 판단을 짚고 다음 장이 필요한 이유를 연결하는 한 문장
- 노트는 슬라이드 문구를 반복하거나 상세 reference를 쌓지 않고 **핵심 전달용 cue**로 작성한다.
  한국어 기본 목표는 장당 약 60초, 80~600자다. notes는 반드시 `질문:`으로 시작하고 전체를
  5문장 이내로 유지한다. `질문`은 20~140자의 한 문장, `핵심 메시지`는 30~260자의 1~3문장,
  `전환`은 180자 이하의 한 문장으로 작성한다.
  - 노트는 보고서 문장을 옮긴 글이 아니라 **발표자가 그대로 읽어도 자연스러운 쉬운 구어체**로 쓴다.
  ...
  - 공식 제품명·기능명은 유지하되, 처음 나오는 technical term은 `trace, 즉 실행 기록`처럼 바로
    ...
    않으며, `승격 여부를 결정합니다`보다 `다음 버전으로 올릴지 정합니다`처럼 말하기 쉬운 표현을 우선한다.
  - 소리 내어 읽었을 때 숨이 차거나 의미가 한 번에 잡히지 않는 문장은 둘로 나눈다. 불필요한
    `~할 수 있습니다`, `~에 해당합니다`, `~을 의미합니다`를 반복하지 않는다.
  - 질문은 단순한 예/아니오 상태 확인으로 끝내지 않는다. 최근 사례·현재 workflow 같은 구체적 맥락과
    evidence·owner·KPI·위험·trade-off·영향 중 하나 이상의 판단 축을 결합해 고객이 설명형으로 답하게
    한다. 고객을 압박하거나 정답을 유도하지 않는다.
  - 핵심 메시지는 제목을 바꿔 말하는 데 그치지 않고, 현재 visual의 핵심 요소 2~4개가 어떻게 연결되어
    결론과 고객 의미를 만드는지 설명한다. 모든 라벨을 낭독하지는 않는다.
  - 전환은 `현재 장의 결론 → 아직 남은 질문·조건 → 다음 장의 역할`을 하나의 bridge로 만든다.
    `다음 장에서 보겠습니다`, `이제 살펴보겠습니다`처럼 어느 장에도 붙일 수 있는 전환은 사용하지 않는다.
    마지막 장은 다음 장을 예고하지 않고 합의할 결정이나 즉시 실행할 행동으로 닫는다.
...
- 모든 슬라이드 notes가 `질문/핵심 메시지/전환`을 포함하고, 질문으로 시작하며, 5문장 이내이고,
  출처·Fact ID·URL 없이 notes QA를 통과한다. 질문은 구체적 맥락과 판단 축을 결합하고, 핵심 메시지는
  visual의 논리를 충분히 설명하며, 전환은 현재 결론과 다음 장의 필요를 자연스럽게 잇는다. 발표자가
  소리 내어 읽었을 때 보고서 문체나 긴 명사 나열 없이 쉬운 구어체로 전달된다.
