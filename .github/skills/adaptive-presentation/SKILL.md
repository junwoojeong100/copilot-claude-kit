...
- 한국어 덱은 모든 슬라이드에 speaker notes를 작성한다. 기존 notes가 있더라도 먼저 완전히 지우고
  ...
  - `질문:` 고객이 자신의 현황을 떠올리게 하는 짧은 질문 한 문장
  - `핵심 메시지:` 발표자가 그대로 말할 수 있는 결론 1~3문장
...
- 노트는 슬라이드 문구를 반복하거나 상세 reference를 쌓지 않고 **핵심 전달용 cue**로 작성한다.
  ...
  `전환`은 180자 이하의 한 문장으로 작성한다. 질문은 고객을 압박하거나 답을 유도하지 않고 현재
  workflow·책임·KPI·위험을 스스로 떠올리게 한다. notes에는 `출처:` 블록, Fact ID, URL을 넣지 않으며,
  근거와 상태는 slide footer·Fact Ledger·deck spec에만 둔다.
...
- 모든 슬라이드 notes가 `질문/핵심 메시지/전환`을 포함하고, 질문으로 시작하며, 5문장 이내이고,
  출처·Fact ID·URL 없이 notes QA를 통과한다.
